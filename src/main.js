...
const createWindow = () => {
	win = new BrowserWindow({
    	...
    	fullscreen: false,
		icon: __dirname + "/UI/assets/MechaChat_256.ico",
		webPreferences: {
			devTools: false,
...
ipc.on('windowClose', (event, data) => {
	const template = [
		{
			label: 'MechaChat',
			icon: __dirname + '/UI/assets/MechaChat_16.png',
			enabled: false
		},
		{
			type: 'separator'
		...
		{
			label: 'Open MechaChat', click: function () {
				win.show();
		...
		{
			label: 'Quit MechaChat', click: function () {
				win.close();
			}
		},
	];
...
function createNewDefaultTray() {
	if (tray == null) {
		tray = new Tray(__dirname + '/UI/assets/MechaChat_16.png');
	}
...
autoUpdater.on('update-available', () => {
	splashWin.webContents.send('update-available-message');
